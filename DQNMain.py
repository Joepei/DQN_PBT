...
def main():
    ...
    with multiprocessing.Manager() as manager:
        Population = manager.list([manager.dict({"theta" : manager.dict({"agent": agent1, "h":manager.list([0.99,64]), "id":1}),\
                                                 "h": manager.list([0.99,64]), "p": manager.dict({"p":0,"id":1}), "t": 0}), \
                                   manager.dict({"theta" : manager.dict({"agent": agent2, "h":manager.list([0.95,16]), "id":2}),\
                                                 "h": manager.list([0.95,16]), "p": manager.dict({"p":0,"id":2}), "t": 0})])
        print("hahaha")
        trainer = algoCode.PBTTrain(step, eval, ready, exploit, explore, update, endofTrain, Population)
        process(trainer, Population)
